...
# 11-2. 생존확률예측 (각 가족 구성원들의)
family_result <- foreach(f=folds) %do% { # 교차 검증을 위한 foreach 반복문
f$train$type <- "T"
f$validation$type <- "V" # type 컬럼을 추가해 훈련데이터와 검증데이터를 분리할 수 있게 함
all <- rbind(f$train, f$validation) # 훈련데이터와 검증데이터를 합쳐 데이터프레임 all 만듦
ctree_model <- ctree( # 훈련데이터만 사용해서 만들되
  survived ~ pclass + sex + age + sibsp + parch + fare + embarked,
  data=f$train)
all$prob <- sapply( # prob는 생존 여부를 예측하는 확률
  predict(ctree_model, type="prob", newdata=all), # predict는 훈련데이터와 검증데이터 모두에 수행
  function(result) { result[1] }) # 생존 확률만 뽑기 위해 사망 확률의 두번재 컬럼을 제외
}

# 11-3. 가족 ID 부여

# 6시까지 작성... 계속 추가 중!! 현재 560 오류 두번 떠서 시간이 오래 걸림 ㅠㅠ...
